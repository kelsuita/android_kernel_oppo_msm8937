		...
		enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		...
		if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_STANDBY)
			akdbgprt("\t[AK4376] %s(%d) dapm->bias_level == SND_SOC_BIAS_STANDBY\n",__FUNCTION__,__LINE__);
		if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_ON)
			akdbgprt("\t[AK4376] %s(%d) dapm->bias_level >= SND_SOC_BIAS_ON\n",__FUNCTION__,__LINE__);
		break;
	case SND_SOC_BIAS_STANDBY:
		if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_PREPARE) {
			akdbgprt("\t[AK4376] %s(%d) dapm->bias_level == SND_SOC_BIAS_PREPARE\n",__FUNCTION__,__LINE__);
			ak4376_pdn_control(codec, 0);
		} if (snd_soc_codec_get_bias_level(codec) == SND_SOC_BIAS_OFF)
			akdbgprt("\t[AK4376] %s(%d) dapm->bias_level == SND_SOC_BIAS_OFF\n",__FUNCTION__,__LINE__);
		break;
...
static struct i2c_driver ak4376_i2c_driver = {
	.driver = {
		.name = "ak4376",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(ak4376_i2c_dt_ids),
	},
