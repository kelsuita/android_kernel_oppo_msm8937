...
static bool touchkey_reverse_enabled = 0;
...
static int synaptics_soft_reset(struct synaptics_ts_data *ts)
{
	...
	if(0 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data(ts->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4A);
	}
	else if(1 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data(ts->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4B);
	}
...
static void gesture_judge(struct synaptics_ts_data *ts)
{
	...
	switch (gesture_sign) {
		case DTAP_DETECT:
			...
			break;
		case SWIPE_DETECT:
			gesture =   (regswipe == 0x41) ? KEY_GESTURE_SWIPE_RIGHT   :
						(regswipe == 0x42) ? KEY_GESTURE_SWIPE_LEFT   :
						(regswipe == 0x44) ? KEY_GESTURE_SWIPE_DOWN      :
						(regswipe == 0x48) ? KEY_GESTURE_SWIPE_UP      :
						(regswipe == 0x80) ? KEY_GESTURE_DOUBLE_SWIPE          :
						KEY_GESTURE_UNKNOWN;
			break;
		case CIRCLE_DETECT:
			gesture = KEY_GESTURE_CIRCLE;
			break;
		case VEE_DETECT:
			gesture =   (gesture_buffer[2] == 0x01) ? KEY_GESTURE_DOWN_VEE  :
						(gesture_buffer[2] == 0x02) ? KEY_GESTURE_UP_VEE    :
						(gesture_buffer[2] == 0x04) ? KEY_GESTURE_RIGHT_VEE :
						(gesture_buffer[2] == 0x08) ? KEY_GESTURE_LEFT_VEE  :
						KEY_GESTURE_UNKNOWN;
			break;
		case UNICODE_DETECT:
			gesture =   (gesture_buffer[2] == 0x77 && gesture_buffer[3] == 0x00) ? KEY_GESTURE_W :
						(gesture_buffer[2] == 0x6d && gesture_buffer[3] == 0x00) ? KEY_GESTURE_M :
						KEY_GESTURE_UNKNOWN;
			break;
		default:
			gesture = KEY_GESTURE_UNKNOWN;
	...
	TPD_ERR("detect %s gesture\n", gesture == KEY_GESTURE_DOUBLE_TAP ? "double tap" :
									gesture == KEY_GESTURE_UP_VEE ? "up vee" :
									gesture == KEY_GESTURE_DOWN_VEE ? "down vee" :
									gesture == KEY_GESTURE_LEFT_VEE ? "(>)" :
									gesture == KEY_GESTURE_RIGHT_VEE ? "(<)" :
									gesture == KEY_GESTURE_CIRCLE ? "circle" :
									gesture == KEY_GESTURE_DOUBLE_SWIPE ? "(||)" :
									gesture == KEY_GESTURE_SWIPE_RIGHT ? "(-->)" :
									gesture == KEY_GESTURE_SWIPE_LEFT ? "(<--)" :
									gesture == KEY_GESTURE_SWIPE_DOWN ? "up to down |" :
...
static void int_key_report(struct synaptics_ts_data *ts)
{
	...
	if (ts->gesture_enable == 1) {
		...
		if( (ret & 0x07) != 0 ){
			gesture_upload = KEY_GESTURE_DOUBLE_TAP;
			input_report_key(ts->input_dev, KEY_GESTURE_DOUBLE_TAP, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_GESTURE_DOUBLE_TAP, 0);
			input_sync(ts->input_dev);
		}	
	} else if (touchkey_enabled) {
		i2c_smbus_write_byte_data(ts->client, 0xff, 0x02);
		ret = i2c_smbus_read_byte_data(ts->client, F1A_0D_DATA00);

		while (i < TOUCHKEY_MAP_MAX) {
			unsigned int mask = touchkey_map[i].mask;
			unsigned int key = touchkey_map[i].keycode;
...
void close_edge_protect_function(struct work_struct *work)
{
	int ret = 0;
	...
	if(0 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data(ts_g->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4A);
	}
	else if(1 == limit_enable)
	{
		ret = i2c_smbus_write_byte_data(ts_g->client, F51_CUSTOM_CTRL_BASE + 0x1D, 0x4B);
	}
	i2c_smbus_write_byte_data(ts_g->client, 0xff, 0x0); 
...
//step 2 :check tx-to-tx and tx-to-vdd
	...
	i2c_smbus_read_i2c_block_data(ts->client, 0x02, TX_NUM, buffer_tx); 
...
#ifdef SUPPORT_GESTURE
	set_bit(KEY_GESTURE_DOUBLE_TAP , ts->input_dev->keybit);//doulbe-tap resume
...
static const struct file_operations tp_touchkey_reverse_enable_proc_fops = {
	.write = tp_touchkey_reverse_write,
	.read =  tp_touchkey_reverse_read,
	.open = simple_open,
	.owner = THIS_MODULE,
};
...
static int finger_protect_handler(void *unused)
{
    ...
    do {
        ...
        wait_event_interruptible(waiter, finger_trigger != FINGER_PROTECT_NOTTRIGGER);
        set_current_state(TASK_RUNNING);
        finger_trigger = FINGER_PROTECT_NOTTRIGGER;
        report_finger_protect = FINGER_PROTECT_NOTREADY;
        do {
            if(i2c_ready == I2C_READY)
                break;
            else
                msleep(5);
        } while(i2c_ready != I2C_READY);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if( (ts->boot_mode == MSM_BOOT_MODE__FACTORY || ts->boot_mode == MSM_BOOT_MODE__RF || ts->boot_mode == MSM_BOOT_MODE__WLAN) )
	{
		...
		v2_8 =  __gpio_get_value(ts->enable2v8_gpio);
		TPD_ERR("v2_8 is %d\n", v2_8);
		return 0;
	}

	INIT_WORK(&ts->close_edge_product, close_edge_protect_function);
	synaptics_read_register_map(ts);
	bootloader_mode = synaptics_rmi4_i2c_read_byte(ts->client, F01_RMI_DATA_BASE);
	bootloader_mode = bootloader_mode&0xff;
	bootloader_mode = bootloader_mode&0x40; 
	...
	strcpy(ts->fw_name,"tp/16017/16017_Firmware_hybrid.img");			
	strcpy(ts->test_limit_name,"tp/16017/16017_Limit_Samsung.img");
	...
	*******************/		
	...
	ret = i2c_smbus_write_byte_data(ts->client, F54_ANALOG_DATA_BASE, 0x03);//select report type 0x02
