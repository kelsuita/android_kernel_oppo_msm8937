...
#include "synaptics_touch_key_s1302_redremote.h"
...
static int tc_hw_pwron(struct synaptics_ts_data *ts)
{
	...
	if( ts->en3v_gpio > 0 ) {
		TPD_ERR("synaptics:enable the en3v_gpio\n");
...
static int tc_hw_pwroff(struct synaptics_ts_data *ts)
{
	...
	if( ts->en3v_gpio > 0 ) {
		TPD_ERR("synaptics:disable  the en3v_gpio\n");
...
extern int is_touch;//form touch lcd 
#ifdef LCD_TRIGGER_KEY_FORCE_CAL
...
static void int_key(struct synaptics_ts_data *ts )
{
	...
	if(ts->pre_btn_state == 0x03)
	{
		ts->pre_btn_state = 0x0;
...
//	int enable_cbc = 0;
	...
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, 0xff, 0x01);
	...
	short_tmp = synaptics_rmi4_i2c_read_byte(ts_g->client, F54_ANALOG_CONTROL_BASE + 0x08);			//close CBC
	short_tmp = short_tmp & 0xEF;
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_CONTROL_BASE + 0x08, short_tmp);

	short_tmp = synaptics_rmi4_i2c_read_byte(ts_g->client, F54_ANALOG_CONTROL_BASE + 0x56);			//close 0D CBC
	short_tmp = short_tmp & 0xEF;
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_CONTROL_BASE + 0x56, short_tmp);
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_CONTROL_BASE + 0x51, 0x01);		//No SignalClarity
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_COMMAND_BASE, 0x04); // force update
	msleep(10);
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, F54_ANALOG_COMMAND_BASE, 0X02);//force Cal
	checkCMD();	
	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_DATA_BASE, 0x07);//select report type 07	
	ret = synaptics_rmi4_i2c_write_word(ts_g ->client, F54_ANALOG_DATA_BASE+1, 0x0);	
	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_COMMAND_BASE, 0x01);//get report
	checkCMD();	
	ret = synaptics_rmi4_i2c_read_block(ts_g ->client, F54_ANALOG_DATA_BASE+3, 2 * TX_NUM * RX_NUM, &(buffer[0]));		//TX_NUM == 2   RX_NUM == 5
	msleep(10);

	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_DATA_BASE, 0x11);//select report type 17	
	ret = synaptics_rmi4_i2c_write_word(ts_g ->client, F54_ANALOG_DATA_BASE+1, 0x0);	
	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_COMMAND_BASE, 0x01);//get report
	checkCMD();	
	ret = synaptics_rmi4_i2c_read_block(ts_g ->client, F54_ANALOG_DATA_BASE+3, 2 * TX_NUM * RX_NUM, &(buffer[20]));
	msleep(10);

	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_DATA_BASE, 0x21);//select report type 33	
	ret = synaptics_rmi4_i2c_write_word(ts_g ->client, F54_ANALOG_DATA_BASE+1, 0x0);	
	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_COMMAND_BASE, 0x01);//get report
	checkCMD();	
	ret = synaptics_rmi4_i2c_read_block(ts_g ->client, F54_ANALOG_DATA_BASE+3, TX_NUM * RX_NUM, &(buffer[40]));		//for RX*TX = 10, we only need half data int report type 33
	msleep(10);
	...
	for(x = 0; x < RX_NUM; x++) {
		...
		for(y = 0; y < RX_NUM; y++) {
			z = 2 * RX_NUM * x + 2 * y;
			short_result = (buffer[z+1]<<8) |buffer[z];
			r_rtest[x][y] = short_result;
			
			if(x == y) {
				//if x == y ,normal area should be [800, 1200], 
				if(short_result < 800 ||short_result > 1200) {
					TPD_ERR("TPD error rx_rx short test failed, short_result = %d\n", short_result);
					num_read_chars += sprintf(&(buf[num_read_chars]), "TPD error rx_rx short test failed, short_result = %d\n", short_result);
					error_count++;
					goto END;
				}
			} else {
				//if x != y , normal area should be [-100, 100]
				if(short_result < -100 ||short_result > 100) {
					TPD_ERR("TPD error rx_rx short test failed, short_result = %d\n", short_result);
					num_read_chars += sprintf(&(buf[num_read_chars]), "TPD error rx_rx short test failed, short_result = %d\n", short_result);
					error_count++;
					goto END;
			...
			printk("%d, ", r_rtest[x][y]);
		}
		printk("\n");
	}

	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_DATA_BASE, 0x05);//select report type 5	
	ret = synaptics_rmi4_i2c_write_word(ts_g ->client, F54_ANALOG_DATA_BASE+1, 0x0);	
	ret = synaptics_rmi4_i2c_write_byte(ts_g ->client, F54_ANALOG_COMMAND_BASE, 0x01);//get report
	checkCMD();	
	ret = synaptics_rmi4_i2c_read_block(ts_g ->client, F54_ANALOG_DATA_BASE+3, 2 , buffer);

	for(x = 0; x < 2; x++) {
		TPD_ERR("TX: %d\n", buffer[x] & 0x01);
		if(buffer[x] & 0x01){
			TPD_ERR("TPD error tx_tx short test failed, tx_buffer = %d\n", buffer[x] & 0x01);
			num_read_chars += sprintf(&(buf[num_read_chars]), "TPD error tx_tx short test failed, tx_buffer = %d\n", buffer[x] & 0x01);
			error_count++;
			goto END;
		}
	}
...
static int tp_reg_address = 0x001C;
static ssize_t tp_reg_operate_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	ret = synaptics_rmi4_i2c_read_block(ts_g->client, tp_reg_address & 0x00ff, 3,&buf[0]);
	ret = synaptics_rmi4_i2c_read_block(ts_g->client, 0x37, 1, &sleep);
	ret = synaptics_rmi4_i2c_read_block(ts_g->client, 0x38, 1, &interrupt);
	ret = synaptics_rmi4_i2c_write_byte(ts_g->client, 0xff, 0x0);
	
	TPD_ERR("read reg=0x%x is 0x%x\n", tp_reg_address,buf[2]);
	ret = sprintf(page, "read reg=0x%x is 0x%x, sleep = 0x%x, interrupt = 0x%x  is_suspend = %d\n", tp_reg_address,buf[2], sleep, interrupt, is_suspend);
...
{    
   ...
   //	delay_qt_ms(60);
	...
	enable_irq(ts_g->irq); //enable_irq(ts_g->client->irq);
...
static const struct file_operations touchkey_reg_operate = {
...
//guomingqiang@phone.bsp 2015-12-17 add for touch key debug node end 
     ...
     proc_entry = proc_create_data("touchkey_reg_operate", 0644, procdir, &touchkey_reg_operate,NULL);	
...
static int synaptics_parse_dts(struct device *dev, struct synaptics_ts_data *ts)
{
	...
	ts->en3v_gpio = of_get_named_gpio(np, "vdd_2v8-supply", 0);
	if( ts->en3v_gpio < 0 ){
...
//	push_component_info(TOUCH_KEY, ts->fw_id, ts->manu_name);
 ...
 //   ret = synaptics_soft_reset(ts);
	...
	TPD_DAILY("synaptics_ts_probe s1302: normal end\n");
	return 0;
...
err_alloc_data_failed:
	...
	ts = NULL;
	ts_g = NULL;
	TPD_ERR("touchkey,s1302 probe error: not normal end ret =%d\n",ret);
	return ret;
}
...
//guomingqiang@phone.bsp 2016-4-15 add for check touch key firmware mode
	...
	if(bootloader_mode == 0x40)
	{
		TPD_ERR(" %s Something terrible wrong , need update the Firmware again\n",__func__);
		#if 0  //only for test 	
		force_update=1;
		ts_g->loading_fw = true;
	...
	{
		TPD_ERR("%s firmware self-check bootloader_mode = 0x%x is ok\n",__func__,bootloader_mode);
	}	
//end	
