 ...
 * Revision 1.2, 2016-10-01, Ping.Liu@BSP.Group.Tp  , modify for the issue of tp error when power off in the middle of update. add TP_TRULY config. 
 ****************************************************************/
...
static void gt1x_select_addr(struct hw_resource *hw_res)
{
...
static int goodix_reset(void *chip_data)
{
    ...
    gt1x_select_addr(chip_info->hw_res);
    gpio_set_int_nopull(chip_info->hw_res);
    msleep(50);
...
static int goodix_get_vendor(void *chip_data, struct panel_info *panel_data)
{
    ...
    if ((id1 == 1) && (id2 == 0) && (id3 == 0)) {
        ...
        strcpy(panel_data->manufacture_info.manufacture, "OFILM");
    } else if ((id1 == 0) && (id2 == 1) && (id3 == 0)) {
        TPD_DEBUG("%s::TP_TPK\n", __func__);
        chip_info->tp_type = TP_BIEL;
        strcpy(panel_data->manufacture_info.manufacture, "BIEL");
        ...
        chip_info->tp_type = TP_OFILM;
        strcpy(panel_data->manufacture_info.manufacture, "Unknown");
    }

    panel_data->tp_type = chip_info->tp_type;
    strcpy(panel_data->fw_name, GT5688_FW_NAME);
...
static fw_check_state goodix_fw_check(void *chip_data, struct resolution_info *resolution_info, struct panel_info *panel_data)
{
    ...
    if (chip_info->tp_type == TP_OFILM)
        panel_data->TP_FW = (ret & 0x000000ff)|0xBC051100;
    if (chip_info->tp_type == TP_BIEL)
        panel_data->TP_FW = (ret & 0x000000ff)|0xBC051600;
    if (chip_info->tp_type == TP_TRULY)
        panel_data->TP_FW = (ret & 0x000000ff)|0xBC051000;
...
static u8 goodix_trigger_reason(void *chip_data, int gesture_enable, int is_suspended)
{
    ...
    TPD_DEBUG("point data is %2x %hhu, vk_status:%hhu\n", ret, result_event, chip_info->vk_status);
...
static void goodix_esd_handle(void *chip_data)
{
    s32 i = 0;
    s32 ret = -1;
    u8 esd_buf[4] = {0};
...
struct goodix_proc_operations goodix_gt5688_proc_ops = {
    .goodix_debug_info_read = goodix_debug_info_read,
