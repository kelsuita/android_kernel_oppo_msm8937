...
/*
 * Copyright (c) 2008-2012 Guangdong OPPO Mobile Comm Corp., Ltd.
 */
...
static int bq27541_soc_calibrate(int soc)
{
	...
	if (bq27541_di->batt_psy) {
		power_supply_get_property(bq27541_di->batt_psy,
		...
		if (ret.intval == POWER_SUPPLY_STATUS_CHARGING || ret.intval == POWER_SUPPLY_STATUS_FULL) { // is charging
			...
			/* set capacity to 100 when full in normal temp */
...
static void bq27541_reset(struct i2c_client *client)
{
	...
	if (bq27541_get_battery_voltage_micros() <= 3300 * 1000
		...
		msleep(150);
		pr_debug("bq27541_reset, point = %d\r\n", bq27541_get_battery_soc());
	} else if (bq27541_di) {
...
static int bq27541_driver_remove(struct i2c_client *client)
{
	...
	if (bq27541_di) {
		...
		udelay(66);
		bq27541_cntl_cmd(bq27541_di->cmd_addr.subcmd_disable_it);
		cancel_delayed_work_sync(&bq27541_di->hw_config);
	}
	return 0;
}
