...
static int __set_clock_rate(struct venus_hfi_device *device,
		unsigned long rate)
{
		...
		struct vidc_clk_scale_data *data)
{
	...
	if (matches) {
		...
		if (cv_info->count) {
			...
			rc = __regulator_set_voltage(device, device->clk_freq, cv_info);
			if (rc) {
				dprintk(VIDC_WARN,
		...
		if (cl->has_scaling) {
			...
			if (!rate) {
				if (!device->clk_bitrate)
					...
					rate = __get_clock_rate_with_bitrate(device,
							load, data,
							instant_bitrate);
			}
			device->clk_freq = rate;
...
static void __flush_debug_queue(struct venus_hfi_device *device, u8 *packet)
{
	struct hfi_packet_header *pkt = NULL;
...
#define SKIP_INVALID_PKT(pkt_size, payload_size, pkt_hdr_size) ({ \
	...
	while (!__iface_dbgq_read(device, packet)) {
		...
		pkt = (struct hfi_packet_header *) packet;
