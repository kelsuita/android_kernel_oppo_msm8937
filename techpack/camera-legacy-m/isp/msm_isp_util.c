...
void legacy_m_msm_isp_print_fourcc_error(const char *origin, uint32_t fourcc_format)
{
	int i;
	char text[5];
	text[4] = '\0';
...
int legacy_m_msm_isp_init_bandwidth_mgr(struct vfe_device *vfe_dev,
...
int legacy_m_msm_isp_update_bandwidth(enum msm_isp_hw_client client,
...
void legacy_m_msm_isp_deinit_bandwidth_mgr(enum msm_isp_hw_client client)
{
...
void legacy_m_msm_isp_util_get_bandwidth_stats(struct vfe_device *vfe_dev,
...
void legacy_m_msm_isp_util_update_clk_rate(long clock_rate)
{
	msm_isp_cpp_clk_rate = clock_rate;
}

uint32_t legacy_m_msm_isp_get_framedrop_period(
...
void legacy_m_msm_isp_get_timestamp(struct msm_isp_timestamp *time_stamp,
	struct vfe_device *vfe_dev)
{
	struct timespec ts;
	do_gettimeofday(&(time_stamp->event_time));
	if (vfe_dev->vt_enable) {
		legacy_m_msm_isp_get_avtimer_ts(time_stamp);
...
int legacy_m_msm_isp_subscribe_event(struct v4l2_subdev *sd, struct v4l2_fh *fh,
	struct v4l2_event_subscription *sub)
{
	return msm_isp_process_event_subscription(fh, sub, true);
}

int legacy_m_msm_isp_unsubscribe_event(struct v4l2_subdev *sd, struct v4l2_fh *fh,
	struct v4l2_event_subscription *sub)
{
	...
	void *arg)
{
	...
	if (fe_cfg->offline_pass == OFFLINE_SECOND_PASS) {
		...
		vfe_dev->hw_info->vfe_ops.core_ops.reset_hw(vfe_dev,
			0, 1);
		legacy_m_msm_isp_reset_framedrop(vfe_dev, stream_info);

		rc = legacy_m_msm_isp_cfg_offline_ping_pong_address(vfe_dev, stream_info,
...
void legacy_m_msm_isp_fetch_engine_done_notify(struct vfe_device *vfe_dev,
	struct msm_vfe_fetch_engine_info *fetch_engine_info)
{
	...
	legacy_m_msm_isp_send_event(vfe_dev, ISP_EVENT_FE_READ_DONE, &fe_rd_done_event);
}
...
int legacy_m_msm_isp_cfg_input(struct vfe_device *vfe_dev, void *arg)
{
	int rc = 0;
...
static int msm_isp_proc_cmd_list_unlocked(struct vfe_device *vfe_dev, void *arg)
{
	...
	rc = legacy_m_msm_isp_proc_cmd(vfe_dev, &proc_cmd->cfg_cmd);
	if (rc < 0)
	...
	while (cmd.next) {
		...
		rc = legacy_m_msm_isp_proc_cmd(vfe_dev, &cmd_next.cfg_cmd);
		if (rc < 0) {
			pr_err("%s:%d failed: rc %d", __func__, __LINE__, rc);
			break;
		}
...
static int msm_isp_proc_cmd_list_compat(struct vfe_device *vfe_dev, void *arg)
{
	...
	rc = legacy_m_msm_isp_proc_cmd(vfe_dev, &current_cmd);
	if (rc < 0)
		pr_err("%s:%d failed: rc %d", __func__, __LINE__, rc);
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_VFE_REG_CFG: {
		...
		rc = legacy_m_msm_isp_proc_cmd(vfe_dev, arg);
	...
	case VIDIOC_MSM_ISP_UNMAP_BUF: {
		...
		rc = legacy_m_msm_isp_proc_buf_cmd(vfe_dev->buf_mgr, cmd, arg);
	...
	case VIDIOC_MSM_ISP_REQUEST_STREAM:
		...
		rc = legacy_m_msm_isp_request_axi_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_RELEASE_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_release_axi_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_CFG_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_cfg_axi_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_AXI_HALT:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_axi_halt(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_AXI_RESET:
		...
		if (atomic_read(&vfe_dev->error_info.overflow_state)
			!= HALT_ENFORCED) {
			rc = legacy_m_msm_isp_stats_reset(vfe_dev);
			rc2 = legacy_m_msm_isp_axi_reset(vfe_dev, arg);
			if (!rc && rc2)
				rc = rc2;
		} else {
	...
	case VIDIOC_MSM_ISP_AXI_RESTART:
		...
		if (atomic_read(&vfe_dev->error_info.overflow_state)
			!= HALT_ENFORCED) {
			rc = legacy_m_msm_isp_stats_restart(vfe_dev);
			rc2 = legacy_m_msm_isp_axi_restart(vfe_dev, arg);
			if (!rc && rc2)
				rc = rc2;
		} else {
	...
	case VIDIOC_MSM_ISP_INPUT_CFG:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_cfg_input(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	...
	case VIDIOC_MSM_ISP_SET_SRC_STATE:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_set_src_state(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_REQUEST_STATS_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_request_stats_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_RELEASE_STATS_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_release_stats_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_CFG_STATS_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_cfg_stats_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_UPDATE_STATS_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_update_stats_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_UPDATE_STREAM:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_update_axi_stream(vfe_dev, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	case VIDIOC_MSM_ISP_SMMU_ATTACH:
		mutex_lock(&vfe_dev->core_mutex);
		rc = legacy_m_msm_isp_smmu_attach(vfe_dev->buf_mgr, arg);
		mutex_unlock(&vfe_dev->core_mutex);
		break;
	...
	case MSM_SD_SHUTDOWN:
		while (vfe_dev->vfe_open_cnt != 0)
			legacy_m_msm_isp_close_node(sd, NULL);
		break;
	...
	unsigned int cmd, void *arg)
{
	...
	case VIDIOC_MSM_VFE_REG_CFG_COMPAT: {
		...
		rc = legacy_m_msm_isp_proc_cmd(vfe_dev, &proc_cmd);
		mutex_unlock(&vfe_dev->realtime_mutex);
		break;
	}
...
long legacy_m_msm_isp_ioctl(struct v4l2_subdev *sd,
	unsigned int cmd, void *arg)
{
	...
	uint32_t *cfg_data, uint32_t cmd_len)
{
	...
	switch (reg_cfg_cmd->cmd_type) {
	case VFE_WRITE: {
		legacy_m_msm_camera_io_memcpy(vfe_dev->vfe_base +
			reg_cfg_cmd->u.rw_info.reg_offset,
	...
	case VFE_WRITE_MB: {
		legacy_m_msm_camera_io_memcpy_mb(vfe_dev->vfe_base +
			reg_cfg_cmd->u.rw_info.reg_offset,
			cfg_data + reg_cfg_cmd->u.rw_info.cmd_data_offset/4,
			reg_cfg_cmd->u.rw_info.len);
		break;
	...
	case VFE_CFG_MASK: {
		...
		temp = legacy_m_msm_camera_io_r(vfe_dev->vfe_base +
			reg_cfg_cmd->u.mask_info.reg_offset);

		temp &= ~reg_cfg_cmd->u.mask_info.mask;
		temp |= reg_cfg_cmd->u.mask_info.val;
		legacy_m_msm_camera_io_w(temp, vfe_dev->vfe_base +
			reg_cfg_cmd->u.mask_info.reg_offset);
		if (grab_lock)
	...
	case VFE_WRITE_DMI_64BIT: {
		...
		for (i = 0; i < reg_cfg_cmd->u.dmi_info.len/4; i++) {
			...
			if (reg_cfg_cmd->cmd_type == VFE_WRITE_DMI_16BIT) {
				...
				legacy_m_msm_camera_io_w(lo_val1, vfe_dev->vfe_base +
				...
				legacy_m_msm_camera_io_w(hi_val, vfe_dev->vfe_base +
					vfe_dev->hw_info->dmi_reg_offset);
			}
			legacy_m_msm_camera_io_w(lo_val, vfe_dev->vfe_base +
				vfe_dev->hw_info->dmi_reg_offset + 0x4);
		}
		break;
	}
	...
	case VFE_READ_DMI_64BIT: {
		...
		for (i = 0; i < reg_cfg_cmd->u.dmi_info.len/4; i++) {
			lo_val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base +
					vfe_dev->hw_info->dmi_reg_offset + 0x4);

			if (reg_cfg_cmd->cmd_type == VFE_READ_DMI_16BIT) {
				lo_val1 = legacy_m_msm_camera_io_r(vfe_dev->vfe_base +
					vfe_dev->hw_info->dmi_reg_offset + 0x4);
				lo_val |= lo_val1 << 16;
			}
			*lo_tbl_ptr++ = lo_val;
			if (reg_cfg_cmd->cmd_type == VFE_READ_DMI_64BIT) {
				hi_val = legacy_m_msm_camera_io_r(vfe_dev->vfe_base +
					vfe_dev->hw_info->dmi_reg_offset);
				*hi_tbl_ptr = hi_val;
				hi_tbl_ptr += 2;
				lo_tbl_ptr++;
		...
		for (i = 0; i < reg_cfg_cmd->u.rw_info.len/4; i++) {
			if ((data_ptr < cfg_data) ||
				...
				(sizeof(*data_ptr) * (data_ptr - cfg_data) >=
				 cmd_len))
				return -EINVAL;
			*data_ptr++ = legacy_m_msm_camera_io_r(vfe_dev->vfe_base +
				reg_cfg_cmd->u.rw_info.reg_offset);
...
int legacy_m_msm_isp_proc_cmd(struct vfe_device *vfe_dev, void *arg)
{
	int rc = 0, i;
...
int legacy_m_msm_isp_send_event(struct vfe_device *vfe_dev,
	uint32_t event_type,
...
int legacy_m_msm_isp_cal_word_per_line(uint32_t output_format,
	uint32_t pixel_per_line)
{
	...
	default:
		legacy_m_msm_isp_print_fourcc_error(__func__, output_format);
		break;
	}
	return val;
}

enum msm_isp_pack_fmt legacy_m_msm_isp_get_pack_format(uint32_t output_format)
{
	switch (output_format) {
	case V4L2_PIX_FMT_SBGGR8:
	case V4L2_PIX_FMT_SGBRG8:
...
int legacy_m_msm_isp_get_bit_per_pixel(uint32_t output_format)
{
	switch (output_format) {
...
void legacy_m_msm_isp_update_error_frame_count(struct vfe_device *vfe_dev)
{
...
static int msm_isp_process_iommu_page_fault(struct vfe_device *vfe_dev)
{
	...
	legacy_m_msm_isp_halt_send_error(vfe_dev, ISP_EVENT_IOMMU_P_FAULT);
	...
	if (vfe_dev->buf_mgr->pagefault_debug_disable == 0) {
		...
		legacy_m_msm_isp_print_ping_pong_address(vfe_dev,
			vfe_dev->page_fault_addr);
		vfe_dev->hw_info->vfe_ops.axi_ops.
...
void legacy_m_msm_isp_process_error_info(struct vfe_device *vfe_dev)
{
	struct msm_vfe_error_info *error_info = &vfe_dev->error_info;
...
void legacy_m_msm_isp_process_overflow_irq(
	struct vfe_device *vfe_dev,
...
void legacy_m_msm_isp_reset_burst_count_and_frame_drop(
	...
	uint32_t ping_pong_status)
{
	...
	legacy_m_msm_isp_get_timestamp(&queue_cmd->ts, vfe_dev);
...
irqreturn_t legacy_m_msm_isp_process_irq(int irq_num, void *data)
{
	...
	legacy_m_msm_isp_process_overflow_irq(vfe_dev,
...
void legacy_m_msm_isp_do_tasklet(unsigned long data)
{
	unsigned long flags;
	struct vfe_device *vfe_dev = (struct vfe_device *) data;
	...
	while (atomic_read(&vfe_dev->irq_cnt)) {
		...
		legacy_m_msm_isp_process_error_info(vfe_dev);
...
int legacy_m_msm_isp_set_src_state(struct vfe_device *vfe_dev, void *arg)
{
...
int legacy_m_msm_isp_open_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	struct vfe_device *vfe_dev = v4l2_get_subdevdata(sd);
	long rc = 0;
	...
	vfe_dev->vfe_hw_version = legacy_m_msm_camera_io_r(vfe_dev->vfe_base);
	...
	legacy_m_cam_smmu_reg_client_page_fault_handler(
...
int legacy_m_msm_isp_close_node(struct v4l2_subdev *sd, struct v4l2_subdev_fh *fh)
{
	long rc = 0;
	int wm;
	struct vfe_device *vfe_dev = v4l2_get_subdevdata(sd);
...
void legacy_m_msm_isp_flush_tasklet(struct vfe_device *vfe_dev)
{
	unsigned long flags;
	struct msm_vfe_tasklet_queue_cmd *queue_cmd;
...
void legacy_m_msm_isp_save_framedrop_values(struct vfe_device *vfe_dev,
				enum msm_vfe_input_src frame_src)
{
	struct msm_vfe_axi_stream *stream_info = NULL;
	uint32_t j = 0;
