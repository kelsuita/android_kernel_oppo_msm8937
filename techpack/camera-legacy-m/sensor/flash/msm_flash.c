	...
	struct msm_flash_cfg_data_t *flash_data)
{
	...
	rc = legacy_m_msm_camera_fill_vreg_params(
		...
		flash_ctrl->power_info.power_setting_size);
	if (rc < 0) {
		pr_err("failed: legacy_m_msm_camera_fill_vreg_params for PON rc %d", rc);
	}
	...
	rc = legacy_m_msm_camera_fill_vreg_params(
		...
		flash_ctrl->power_info.power_down_setting_size);
	if (rc < 0) {
		pr_err("failed: legacy_m_msm_camera_fill_vreg_params for PDOWN rc %d", rc);
	}
#endif
...
static int flash_proc_init(struct msm_flash_ctrl_t *flash_ctl)
{
	...
	/* Parse and fill vreg params for powerup settings */
	ret = legacy_m_msm_camera_fill_vreg_params(
		power_info->cam_vreg,
		power_info->num_vreg,
		power_info->power_setting,
		power_info->power_setting_size);
	if (ret < 0) {
		pr_err("failed: legacy_m_msm_camera_fill_vreg_params for PUP ret %d", ret);
	}
	...
	if (ret < 0) {
		pr_err("failed: legacy_m_msm_camera_fill_vreg_params for PDOWN ret %d", ret);
	}
