...
void legacy_m_msm_camera_io_w(u32 data, void __iomem *addr)
{
	CDBG("%s: 0x%pK %08x\n", __func__,  (addr), (data));
...
* to same address
*/
int32_t legacy_m_msm_camera_io_w_block(const u32 *addr, void __iomem *base,
	u32 len)
{
	int i;
...
int32_t legacy_m_msm_camera_io_w_reg_block(const u32 *addr, void __iomem *base,
	u32 len)
{
	int i;
...
void legacy_m_msm_camera_io_w_mb(u32 data, void __iomem *addr)
{
	CDBG("%s: 0x%pK %08x\n", __func__,  (addr), (data));
...
int32_t legacy_m_msm_camera_io_w_mb_block(const u32 *addr, void __iomem *base, u32 len)
{
	int i;
...
u32 legacy_m_msm_camera_io_r(void __iomem *addr)
{
...
u32 legacy_m_msm_camera_io_r_mb(void __iomem *addr)
{
	uint32_t data;
	/* ensure read is done */
	rmb();
...
void legacy_m_msm_camera_io_memcpy_toio(void __iomem *dest_addr,
...
int32_t legacy_m_msm_camera_io_poll_value(void __iomem *addr, u32 wait_data, u32 retry,
	unsigned long min_usecs, unsigned long max_usecs)
{
	...
	tmp = legacy_m_msm_camera_io_r(addr);
...
int32_t legacy_m_msm_camera_io_poll_value_wmask(void __iomem *addr, u32 wait_data,
...
void legacy_m_msm_camera_io_dump(void __iomem *addr, int size, int enable)
{
...
void legacy_m_msm_camera_io_dump_wstring_base(void __iomem *addr,
...
void legacy_m_msm_camera_io_memcpy(void __iomem *dest_addr,
	void __iomem *src_addr, u32 len)
{
	CDBG("%s: %pK %pK %d\n", __func__, dest_addr, src_addr, len);
	legacy_m_msm_camera_io_memcpy_toio(dest_addr, src_addr, len / 4);
}

void legacy_m_msm_camera_io_memcpy_mb(void __iomem *dest_addr,
	void __iomem *src_addr, u32 len)
{
	int i;
	u32 *d = (u32 *) dest_addr;
	...
	wmb();
	for (i = 0; i < (len / 4); i++) {
		legacy_m_msm_camera_io_w(*s++, d++);
		/* ensure write is done after every iteration */
		wmb();
	}
}

int legacy_m_msm_cam_clk_sel_src(struct device *dev, struct msm_cam_clk_info *clk_info,
...
int legacy_m_msm_cam_clk_enable(struct device *dev, struct msm_cam_clk_info *clk_info,
...
int legacy_m_msm_camera_config_vreg(struct device *dev, struct camera_vreg_t *cam_vreg,
...
int legacy_m_msm_camera_enable_vreg(struct device *dev, struct camera_vreg_t *cam_vreg,
		int num_vreg, enum msm_camera_vreg_name_t *vreg_seq,
...
void legacy_m_msm_camera_bus_scale_cfg(uint32_t bus_perf_client,
...
int legacy_m_msm_camera_set_gpio_table(struct msm_gpio_set_tbl *gpio_tbl,
...
int legacy_m_msm_camera_config_single_vreg(struct device *dev,
...
int legacy_m_msm_camera_request_gpio_table(struct gpio *gpio_tbl, uint8_t size,
	int gpio_en)
{
	int rc = 0, i = 0, err = 0;
...
/*
 * legacy_m_msm_camera_get_dt_reg_settings - Get dt reg settings from device-tree.
 ...
 */
int legacy_m_msm_camera_get_dt_reg_settings(struct device_node *of_node,
...
/*
 * legacy_m_msm_camera_get_dt_reg_settings - Free dt reg settings memory.
 * @reg_s: Double pointer will be allocated by this function and filled.
 * @size: Pointer to set the length as invalid.
 */
void legacy_m_msm_camera_put_dt_reg_settings(uint32_t **reg_s,
	unsigned int *size)
{
	kfree(*reg_s);
	*reg_s = NULL;
...
int legacy_m_msm_camera_hw_write_dt_reg_settings(void __iomem *base,
	uint32_t *reg_s,
	unsigned int size)
{
	int32_t rc = 0;
	...
	rc = legacy_m_msm_camera_io_w_reg_block((const u32 *) reg_s,
		base, size);
	if (rc < 0)
